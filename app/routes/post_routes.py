...
async def create_post(
    post: CreatePost, service: Annotated[PostService, Depends(get_post_service)]
) -> ShowPost:
    return await service.create_post(post)


@post_router.get("/all/")
async def get_post_all(service: Annotated[PostService, Depends(get_post_service)]):
    return await service.get_post_all()


@post_router.get("/{post_id}/")
async def get_post_by_id(
    post_id: UUID, service: Annotated[PostService, Depends(get_post_service)]
):
