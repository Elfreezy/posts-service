...
async def test_session(test_engine):
    sessionmaker_local = async_sessionmaker(test_engine, class_=AsyncSession, expire_on_commit=False)
...
async def test_client(test_session) -> AsyncGenerator[AsyncClient, None]:
    ...
    async def override_redis_client():
        redis_client = Redis(host=test_settings.TEST_REDIS_HOST, port=test_settings.TEST_REDIS_PORT)
        yield redis_client
    ...
    async with AsyncClient(transport=transport, base_url=test_settings.TEST_CLIENT_BASE_URL) as async_client:
        yield async_client
